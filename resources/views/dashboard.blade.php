...
<div style="display: flex; background-color:rgb(182, 104, 104);" >
        ...
        @if(session('imageName'))
            ...
            <div>
                ...
                <form id="imageForm" action="{{ route('update.image', ['imageName' => session('imageName')]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <button type="button" onclick="document.getElementById('imageInput1').click();">Seleccionar archivo</button>
    ...
    <div>
        ...
        @if(session('imageName2') || session()->has('imageName2'))
            <img src="{{ asset('assets/images/backgrounds/' . session('imageName2')) }}" alt="nizon" class="imagen-about-us" style="width: 120px;">
            <div>
                <a href="{{ route('delete.image2', ['imageName2' => session('imageName2')]) }}">Eliminar</a>
                <!-- Agrega un formulario para "Actualizar" -->
                <form id="imageForm2" action="{{ route('update.image2', ['imageName2' => session('imageName2')]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="file" id="imageInput2" name="new_image" style="display: none;">
                    <button type="button" onclick="document.getElementById('imageInput2').click();">Seleccionar archivo</button>
    ...
    <div>
        ...
        @if(session('imageName3') || session()->has('imageName3'))
            ...
            <img src="{{ asset('assets/images/backgrounds/' . session('imageName3')) }}" alt="nizon" class="imagen-about-us" style="width: 120px;">
            <div>
                <a href="{{ route('delete.image3', ['imageName3' => session('imageName3')]) }}">Eliminar</a>
                <form id="imageForm3" action="{{ route('update.image3', ['imageName3' => session('imageName3')]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
         
                    <input type="file" id="imageInput3" name="new_image" style="display: none;">
                    <button type="button" onclick="document.getElementById('imageInput3').click();">Seleccionar archivo</button>
